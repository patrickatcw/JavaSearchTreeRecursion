...
public class SearchTree implements NodeList{
    ...
    public boolean removeItem(ListItem item) {
        //step 42 added....determining the items to be removed
        if (item != null){
    ...
    //step 43 method to actually remove items
    private void performRemoval(ListItem item, ListItem parent){

        //remove item from the tree
        if (item.next()== null) {
            //no right tree, so mkae parent point to the left tree(which may be null)
            if (parent.next() == item){
                //item is rright child of its parent
